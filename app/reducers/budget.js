...
export default function(state: BudgetState = initialState, action: Action): BudgetState {
  switch(action.type) {
    ...
    case GET_BUDGET_TOTAL_REQUEST: return getBudgetTotalRequest(state, action)
    case GET_BUDGET_TOTAL_SUCCESS: return getBudgetTotalSuccess(state, action)
    case GET_BUDGET_TOTAL_FAILURE: return getBudgetTotalFailure(state, action)
...
function getBudgetTotalRequest(
  ...
  return Object.assign({}, state, { isFetchComplete: false })
}

function getBudgetTotalSuccess(
    state: BudgetState, action: GetBudgetTotalSuccessAction): BudgetState {
  return Object.assign({}, state, { total: action.total, isFetchComplete: true, errorMessage: "" })
}

function getBudgetTotalFailure(
    state: BudgetState, action: GetBudgetTotalFailureAction): BudgetState {
  return Object.assign({}, state, { errorMessage: action.errorMessage, isFetchComplete: true })
}
